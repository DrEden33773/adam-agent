...
test.each(["aggregate", "record"] as const)(
  "production managed review and Trash retain %s evidence through real codecs, JSONL and provider boundaries",
...
export function activate(context) {
  ...
  context.registerOperation({ id: "fixture.runtime-review", input: extensionProjectChangeSnapshotCodec, output: codec("fixture.output"), progress: codec("fixture.progress"), managedOutput: { ...codec("fixture.verdict"), decode: value => value?.verdict === "verified" ? { ok: true, value } : { ok: false, issues: [] } },
    async execute(input, operation) {
      ...
      const record = await operation.capabilities["adam.storage.records@1"].create({ key: "review-source", contract: { id: "fixture.review-source", version: 1 }, value: "Retained review source record" });
      const decoded = extensionManagedReviewTerminalCodec.decode(await operation.capabilities["adam.managed-review@1"].review({ evidence: [{ type: "artifact", artifact }, { type: "record", record }], instruction: "Review immutable capture and return JSON.", outputContract: { id: "fixture.verdict", version: 1 } }));
...
`,
      ...
      const admission = (await controlStore.forParent(sessionId).read()).find(
      ...
      if (admission?.event.type !== "admitted" || admission.event.frozen?.review === undefined)
        throw new Error("Missing durable review evidence descriptor");
      let evidencePath = join(
        stateRoot,
        "artifacts",
        admission.event.frozen.review.evidence.id.slice("sha256:".length),
      );
      if (missingEvidence === "record") {
        const projectRecords = join(
          stateRoot,
          "extension-records",
          trust.projectId.slice("sha256:".length),
        );
        const namespaces = await readdir(projectRecords);
        expect(namespaces).toHaveLength(1);
        const namespace = join(projectRecords, namespaces[0] as string);
        const files = (await readdir(namespace)).filter((name) => name.endsWith(".json"));
        expect(files).toHaveLength(1);
        evidencePath = join(namespace, files[0] as string);
      }
      const evidenceBytes = await readFile(evidencePath);
      const callsBeforeRestore = requests.length;
      const preview = await presentation.dispatch({ type: "preview_session_trash", sessionId });
      ...
      const trashed = presentation.getState().authoritative.sessions.trash?.items[0];
      if (trashed === undefined) throw new Error("Missing reviewer Trash unit");
      ...
      const partial = presentation.getState().authoritative.sessions.trash?.items[0];
      expect(partial?.phase).toBe("restoring");
      await expect(runtime.inspectSession(sessionId)).rejects.toMatchObject({
        code: "session_in_trash",
      });
      await writeFile(evidencePath, evidenceBytes, { mode: 0o400 });
      if (partial === undefined) throw new Error("Missing incomplete reviewer restore");
      expect(
        (
          await presentation.dispatch({
            type: "continue_session_trash",
            transactionId: partial.transactionId,
            expectedRevision: partial.revision,
          })
        ).status,
      ).toBe("admitted");
      expect(requests).toHaveLength(callsBeforeRestore);
    } finally {
      await runtime?.close();
