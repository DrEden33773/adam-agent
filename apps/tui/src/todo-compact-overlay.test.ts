...
test("TodoCompactOverlay bounds unfinished rows, drops completed linger first and collapses", () => {
  const overlay = new TodoCompactOverlay(createAdamTuiTheme(true));
  ...
  overlay.setState({ items, sessionId: "session-a", summary, turnKey: "turn-a" });
