...
test("the extension API release verifier accepts its exact tagged checkout", async () => {
  ...
  await expect(
    verifyRelease({
      GITHUB_REF_NAME: "extension-api-v0.0.0-bootstrap.0",
      GITHUB_REF_TYPE: "tag",
      GITHUB_SHA: commit,
    }),
...
test.each([
  ...
  {
    environment: {
      GITHUB_REF_NAME: "extension-api-v0.0.0-bootstrap.1",
      GITHUB_REF_TYPE: "tag",
    },
  ...
  await expect(
    ...
    stderr: expect.stringContaining(
      "Release must run from the exact tag extension-api-v0.0.0-bootstrap.0.",
    ),
  });
});
