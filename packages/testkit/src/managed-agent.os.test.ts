...
test("ManagedAgentStore reads a cold A2 background admission without rewriting its historical truth", async () => {
  ...
  try {
    ...
    const cold = await createJsonlManagedAgentStore({ stateRoot, workspaceRoot });
    const records = await cold.read();
    expect(managedAgentSnapshotFromRecords(records, admission.parentSessionId)).toMatchObject({
      agents: [{ agentId: admission.agentId, status: "running" }],
    });
    await expect(cold.read()).resolves.toEqual([admission]);
  } finally {
    await rm(testRoot, { recursive: true, force: true });
  }
});
