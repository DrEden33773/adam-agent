...
The package retains historical `0.3.0`, `0.4.0`, and `0.5.0` contract declarations for record compatibility. The ordinary Adam Host uses `adam.managed-review@1` and refuses retired `adam.managed-session@1` / `@2` execution requests before extension import. Historical records remain readable; declaring or granting an old capability does not restore its executor.
