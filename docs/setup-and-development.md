...
pnpm tui --target deepseek-flash.direct
```
...
The example selects `deepseek-flash.direct` (DeepSeek V4.1 Flash), the recommended text-and-image target. Select Model also offers `deepseek-v4-pro.direct` for text-only work. Flash target profile v4 uses Adam's bounded Direct Responses SSE driver for ordinary calls, titles and compaction; attached images are read through `read_input_resource` and returned as image tool results. Pro and legacy Flash Chat profiles use the dedicated `@ai-sdk/deepseek@3.0.30` implementation over the public `@ai-sdk/provider@4.0.7` `LanguageModelV4.doStream()` contract. Both routes send requests directly to `https://api.deepseek.com`.

The old `deepseek-v4-flash.direct` and `deepseek-v4-flash-vision-exp.direct` targets remain valid for explicit configuration, saved defaults and historical sessions, but are hidden from Select Model. Adam preserves their recorded identities and profiles; the [DeepSeek service now routes both old call names to V4.1 Flash](https://api-docs.deepseek.com/quick_start/pricing/). Legacy Vision profile v2 keeps its Responses image-tool path, and historical Vision profile v1 keeps its Chat image-input path. The `ADAM_AGENT_PROVIDER=deepseek` plus exact `ADAM_AGENT_MODEL` compatibility selectors accept `deepseek-flash`, `deepseek-v4-flash`, `deepseek-v4-pro`, and `deepseek-v4-flash-vision-exp`; they cannot be combined with `ADAM_AGENT_TARGET`, and omitting the legacy model still selects Pro.

Adam does not persist credentials, provider response IDs, provider conversation state, or raw provider responses. Schema v3 stores only bounded normalized replay-critical reasoning with the complete response envelope in owner-only session state; Adam-owned reasoning start and settlement events are canonical, cumulative text updates are live-only, and the browser-safe Presentation projection preserves explicit owner-only disclosure instead of merging reasoning into an assistant answer. Provider failures are reduced to bounded Adam-owned metadata before session persistence. The live model uses the same recorded tool permissions. New sessions allow exact built-in Todo bookkeeping by default; filesystem writes, execution and delegation retain their own permission requirements. Historical sessions retain their recorded policy until explicitly upgraded.
