...
- The versioned `plan-policy.hybrid-delegation-v1` policy also admits the exact built-in non-mutating managed-control adapters under the same current role, permission, target, and budget ceilings. Plan does not grant delegation authority, transfer its approval to a child, or revise a ready artifact when child evidence arrives. Historical read and hybrid policies retain their original profiles.
- New Todo-enabled hybrid Plan policies admit only the exact built-in session Todo create/update/batch operations under the recorded Todo permission policy, ordinary explicit denials and role ceilings. Todo remains a write effect. Historical Plan versions and sessions without a Todo permission version retain their original semantics; an explicit idle upgrade affects future calls and new Plan cycles, not pending requests or existing Plan artifacts.
