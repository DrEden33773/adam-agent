...
The repository also contains the source-level `@adam-agent/extension-api` contract and a trusted in-process Extension Host foundation. The Host loads only explicitly configured package roots, validates locked identity, version, compatibility, capability grants, and confined ESM entry points before activation, publishes contribution registrations transactionally, and persists enable or disable state before changing visibility. Extension JavaScript runs with the Adam process's authority: this is not a package installer, marketplace, or security sandbox. Configured package roots are trusted mutable code; the Host validates their call-time state but does not snapshot them or contain concurrent same-user mutation. Lifecycle appends are serialized across Host instances inside one Adam process and do not claim cross-process shared-Host coordination. The extension package is not yet published to npm, and this foundation is not wired into the default CLI until the remaining operation and capability checkpoints are complete.
